...
(function (root, factory) {
  ...
  function normName(n) {
    ...
    // See-through V3 emits explicit side suffixes such as eyewhite-l / eyewhite-r.
    // Auto-live2D's side-aware slots already split/anchor by geometry, so normalize
    // these aliases back to their semantic base names before slot matching.
    if (/^(eyewhite|irides|eyelash|eyebrow|ears|handwear)[-_](l|r)$/.test(n)) {
      n = n.replace(/[-_](l|r)$/, '');
    }
    return n;
  }
